...
class SNN(SN10):
    def __init__(self, num_hidden_units: int, input_dim: int = 11 * 20, activation_function='relu', dropout_prob: float = 0):
...
class DNN(NCOModel, nn.Module):
    def __init__(self, num_hidden_units: tuple, activation_function='relu', input_dim: int = 11 * 20, dropout_prob: float = 0):
        super(DNN, self).__init__()
        self.flatten = nn.Flatten()
        self.linear_1 = nn.Linear(input_dim, num_hidden_units[0])
        self.dropout_1 = nn.Dropout(dropout_prob)
        self.activation = activation_functions.get(activation_function, nn.ReLU())
        self.linear_2 = nn.Linear(num_hidden_units[0], num_hidden_units[1])
        self.dropout_2 = nn.Dropout(dropout_prob)
        self.linear_3 = nn.Linear(num_hidden_units[1], 1)
        self.final = nn.Sigmoid()
